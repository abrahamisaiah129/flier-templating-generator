...
export function NewPropertyView({
  ...
  // Active Brief Tab state for unclustered brief input when multiple briefs exist
  const [activeBriefTab, setActiveBriefTab] = useState<number>(0);
  const safeBriefTab = Math.min(Math.max(0, activeBriefTab), Math.max(0, briefs.length - 1));
  ...
  const handleRemoveBrief = (index: number) => {
    if (briefs.length > 1) {
      ...
      setActiveBriefTab((prev) => {
        if (prev >= briefs.length - 1) {
          return Math.max(0, briefs.length - 2);
        }
        return prev;
      });
    }
  ...
  return (
    <div className="max-w-3xl mx-auto py-4 px-2 sm:px-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm mb-6 transition-all">
        ...
        {/* Multi-Brief Tabs when multiple briefs exist */}
        {briefs.length > 1 ? (
          <div>
            {/* Tabs Header */}
            <div className="flex items-center gap-2 mb-3 overflow-x-auto pb-1">
              {briefs.map((b, idx) => {
                const isActive = safeBriefTab === idx;
            ...
            <div className="relative">
              <textarea
                value={briefs[safeBriefTab] || ""}
                onChange={(e) => handleBriefChange(safeBriefTab, e.target.value)}
                rows={5}
                placeholder={
                  safeBriefTab === 0
                    ? "Paste raw text, WhatsApp messages, or notes here. We'll automatically structure the details for your flyer."
                    : `Paste raw text, WhatsApp messages, or notes for brief ${safeBriefTab + 1} here...`
                }
          ...
          <div>
            <textarea
              value={briefs[0] || ""}
              onChange={(e) => handleBriefChange(0, e.target.value)}
              rows={5}
